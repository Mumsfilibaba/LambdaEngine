...
#define MAX_UNIQUE_MATERIALS 64
...
layout(binding = 4, set = 0) uniform sampler2D u_SceneRoughnessMaps[MAX_UNIQUE_MATERIALS];
...
void main()
{
	...
	vec4 sampledAlbedoAll 	= texture(u_SceneAlbedoMaps[in_MaterialIndex], texCoord);
	if (sampledAlbedoAll.a < 0.4f)
	{
		discard;
	}

	vec3 sampledAlbedo 	    = pow(sampledAlbedoAll.rgb, vec3(GAMMA));
	...
	vec3 	sampledSpecular 	= texture(u_SceneRoughnessMaps[in_MaterialIndex], texCoord).rgb;
	float 	sampledAO 			= sampledSpecular.r;
	float 	sampledRoughness  	= sampledSpecular.g;
	float 	sampledMetallic 	= sampledSpecular.b;

	sampledNormal 		= vec3(1.0f) - sampledNormal;
	//sampledNormal.z 	= 1.0f - sampledNormal.z;
